...
14. [Schema (Structured Data)](#schema-structured-data)
15. [Decision Log](#decision-log)
16. [Future Enhancements](#future-enhancements)
17. [Handoff Notes for New Collaborators](#handoff-notes-for-new-collaborators)
...
All structured data on trust-lionel.com is implemented as JSON-LD, injected via Astro's `set:html` directive in `src/layouts/Layout.astro`. The schema was validated at Google's Rich Results Test and currently shows **0 errors, 0 warnings** for both the homepage (Person + WebSite) and all post pages (BlogPosting).

**Validation URLs:**
- Homepage: `https://search.google.com/test/rich-results?url=https://trust-lionel.com`
- Post pages: `https://search.google.com/test/rich-results?url=https://trust-lionel.com/posts/cloud-migration`

**Validation status as of June 22, 2026:**
| Schema Type | Pages | Status |
|-------------|-------|--------|
| Person | All pages | ✅ 0 errors, 0 warnings |
| WebSite | All pages | ✅ 0 errors, 0 warnings |
| BlogPosting | Post pages only | ✅ 0 errors, 0 warnings |

---

### Block 1 — Person Schema (Every Page)

Injected in `src/layouts/Layout.astro`. Appears on every page of the site.

**Removed properties and reasons:**
- `maintainer` — Google flagged `SoftwareApplication` and `SoftwareSourceCode` as invalid in the Person context
- `author` — Not a recognized property for the Person type in schema.org
...
```json
{
  ...
  "image": "https://trust-lionel.com/og-image.png",
  "jobTitle": "IT Consultant & Innovative Thought Leader",
  "description": "IT Consultant, Cloud Engineer, Innovative Thought Leader, and Subject Matter Expert specializing in cloud infrastructure, cybersecurity, Microsoft 365, and enterprise IT governance. Based in Houston, TX — serving organizations nationwide through 4TH AND BAILEY.",
  "address": {
    "@type": "PostalAddress",
  ...
  "worksFor": {
    "@type": "Organization",
    "name": "4TH AND BAILEY",
    "url": "https://4thandbailey.com"
  },
  "sameAs": [
    "https://trust-lionel.com",
    "https://www.lionelmosley.com",
    "https://linkedin.com/in/lionelmosley",
    "https://github.com/trust-lionel",
    "https://youtube.com/@LMosley002",
    "https://x.com/lionelmosley",
    "https://www.facebook.com/yourinfotechconsultant",
    "https://www.threads.com/@lionelmosley",
    "https://bsky.app/profile/lionelmosley",
    "https://instagram.com/lionelmosley",
    "https://reddit.com/u/trust-lionel",
    "https://open.spotify.com/user/lmosley002-spotify",
    "https://about.me/lionelmosley",
    "https://blog.lionelmosley.com",
    "https://devops.houstonalert.com",
    "https://wordcards.co"
  ],
  "knowsAbout": [
...
### Block 2 — WebSite Schema (Every Page)

Injected in `src/layouts/Layout.astro`. Appears on every page of the site immediately after the Person schema.
...
### Block 3 — BlogPosting Schema (Post Pages Only)

Injected conditionally in `src/layouts/Layout.astro` when the `blogPosting` prop is passed from `src/pages/posts/[...id].astro`. Values are derived dynamically from each post's frontmatter at build time.

**Template with field sources:**
...
```json
{
  ...
  "datePublished": "[frontmatter.pubDate.toISOString().split('T')[0]]",
  "url": "[https://trust-lionel.com/posts/{post.id}]",
  "image": "[frontmatter.cover URL or /og-image.png fallback]",
  "keywords": "[frontmatter.tags array]"
}
```

**How it is injected in `src/pages/posts/[...id].astro`:**
...
**Production examples — validated BlogPosting entries:**

| Post | datePublished | Validation |
|------|--------------|------------|
| cloud-migration | 2026-06-16 | ✅ 0 errors, 0 warnings |
| microsoft-notification-abuse | 2026-05-22 | ✅ 0 errors, 0 warnings |
| three-questions | 2026-05-17 | ✅ 0 errors, 0 warnings |
| vibe-coding | 2026-05-16 | ✅ 0 errors, 0 warnings |
| macsystools | 2026-03-05 | ✅ 0 errors, 0 warnings |

---

### Schema Injection Method

The schema is injected using Astro's `set:html` directive rather than `is:inline`. This is a critical implementation detail:

```astro
<!-- CORRECT — evaluates JSON.stringify() at build time -->
<script is:inline type="application/ld+json" set:html={JSON.stringify({...})} />

<!-- INCORRECT — renders the literal string "{JSON.stringify({...})}" as text -->
<script is:inline type="application/ld+json">{JSON.stringify({...})}</script>
```

The `is:inline` directive is required alongside `set:html` to silence Astro's warning about script processing. Without `is:inline`, Astro attempts to process the script through its bundler, which is not appropriate for JSON-LD.

---

### www.lionelmosley.com — Current Configuration and Future Enhancement

#### Current Configuration

`www.lionelmosley.com` is currently a CDN-level redirect to `trust-lionel.com` via BunnyCDN. It is not a separate deployment. All traffic arriving at `www.lionelmosley.com` is transparently redirected to `https://trust-lionel.com`.

**Current schema treatment:**
`https://www.lionelmosley.com` is included in the `sameAs` array of the Person schema, which signals to Google that both domains represent the same entity (Lionel Mosley). This is the correct approach for a redirect relationship.

**To add `www.lionelmosley.com` to the `sameAs` array** in `src/layouts/Layout.astro`, add the following entry to the `sameAs` array immediately after `"https://trust-lionel.com"`:

```json
"https://www.lionelmosley.com"
```

#### Future Enhancement — Decoupling to a Standalone Site

If `www.lionelmosley.com` is ever decoupled from `trust-lionel.com` and becomes its own standalone site, the following schema and configuration changes will be required:

**Step 1 — Determine the canonical domain.**
Decide whether `trust-lionel.com` or `lionelmosley.com` becomes the primary canonical domain. The canonical domain should be the one that receives the most inbound links and organic traffic. The Person schema `"url"` field should point to the canonical domain.

**Step 2 — Update `trust-lionel.com` Person schema.**
Change `sameAs` treatment: `www.lionelmosley.com` remains in the `sameAs` array but the relationship becomes peer-to-peer rather than redirect-to-canonical.

**Step 3 — Add schema to `lionelmosley.com`.**
The standalone `lionelmosley.com` site will need its own Person schema and WebSite schema:
...
```json
{
  ...
  "@type": "Person",
  "name": "Lionel Mosley",
  "url": "https://www.lionelmosley.com",
  "sameAs": [
    "https://trust-lionel.com",
    "https://linkedin.com/in/lionelmosley"
    // ... all existing sameAs entries
  ]
}
```
...
**Step 4 — Update BunnyCDN configuration.**
Remove or reconfigure the BunnyCDN redirect rules. If `lionelmosley.com` runs on its own deployment, the CDN should route traffic to the new origin rather than redirecting to `trust-lionel.com`.

**Step 5 — Add canonical link tags.**
Each page on `lionelmosley.com` will need a `<link rel="canonical">` tag pointing to its own URL, not to `trust-lionel.com`.

**Step 6 — Notify Google Search Console.**
Submit the `lionelmosley.com` sitemap to Google Search Console separately and monitor for indexing of the new domain.

**Step 7 — Validate schema on both domains.**
After decoupling, validate both `https://trust-lionel.com` and `https://www.lionelmosley.com` at `https://search.google.com/test/rich-results` and confirm 0 errors, 0 warnings on each.
