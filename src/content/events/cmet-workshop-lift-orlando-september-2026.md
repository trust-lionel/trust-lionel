---
title: "Your Website Should Be Making You Money"
description: "A live, interactive workshop for entrepreneurs on building a revenue-generating website — led by Lionel Mosley, IT Consultant and Innovative Thought Leader. Hosted by Lift Orlando in collaboration with The Center for Micro-Entrepreneurial Training (CMET)."
eventDate: 2026-09-24
eventTime: "6&#58;00 PM - 8&#58;00 PM EDT"
eventFormat: "In-Person"
eventHost: "Lift Orlando"
eventHostUrl: "https://www.liftorlando.org/"
eventStatus: "upcoming"
eventSameAs: "https://github.com/cmet-lionel/ai-github-pages-workshop"
eventSecondaryButtonLabel: "Lift Orlando"
eventSecondaryButtonUrl: "https://www.liftorlando.org/"
eventPrimaryButtonLabel: "Learn About CMET"
eventPrimaryButtonUrl: "https://tcmet.org/"
tags: ["Entrepreneurship", "Small Business", "Website Strategy", "Digital Transformation"]
featured: true
---

*Hosted by Lift Orlando in collaboration with The Center for Micro-Entrepreneurial Training (CMET)*

---

> "The most valuable page on the internet for your business is the one that makes a stranger trust you before they ever call you. Tonight, you are going to build that page. With AI. For free. And you will own it permanently."

## This Workshop Is Designed For You

The entrepreneurs in this room run businesses where trust is the product. Before a client books the appointment, places the catering order, enrolls their child, or calls the midwife — they have to believe in the person behind the business. This workshop was built for exactly that kind of business.

Expected attendees include entrepreneurs in phlebotomy, catering, daycare, event planning, home education, veteran housing, K–8 learning academies, midwifery, notary services, party and event planning, and custom apparel. No technical background required. Every step of the workshop runs on a smartphone or laptop using tools that are free to use tonight and free to keep forever.

## What You Will Build

In two hours, you will use Claude — the AI tool at [claude.ai](https://claude.ai) — to write the content for your business website. Then you will publish that website using GitHub Pages, a free platform used by developers at companies like Google, Microsoft, and Amazon. You will leave tonight with a live website at a real URL that you own — not rented from Wix, not dependent on a subscription, not subject to someone else's pricing decision.

> "SCORE will give you a mentor. LinkedIn will rent you an audience. Wix will host your website — until they decide to change their pricing. Tonight you are going to learn what it means to own your platform, own your data, and own your stack."

## What You Need

- A smartphone or laptop
- A free account at [claude.ai](https://claude.ai)
- A free account at [github.com](https://github.com)
- Your business name and a one-sentence description of what you do

That is it. Everything else happens in the room.

## Why CMET Built This

> "CMET built this workshop because we believe that the tools that power billion-dollar companies should be accessible to every entrepreneur in this room. Tonight, they are."

The Center for Micro-Entrepreneurial Training exists to close the gap between the technology available to large organizations and the technology available to the entrepreneurs who need it most. This workshop is one expression of that mission — free to attend, practical from the first minute, and built around tools you will still be using five years from now.

---

**When:** Thursday, September 24, 2026 · 6:00 PM – 8:00 PM EDT

**Where:** Lift Orlando · 710 S Tampa Ave Suite 209, Orlando, FL 32805

**Facilitated by:** Lionel Mosley, Senior Consultant — 4TH AND BAILEY

[**→ Join CMET Nation**](https://cmetnation.org/login)
